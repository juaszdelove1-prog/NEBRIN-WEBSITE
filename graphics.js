...
const terms=["Graphics", "Design", "Branding", "Digital Marketing"];
async function init(){if(!await getProfile(["CEO", "Super Admin", "Manager", "Graphics", "Graphic Designer"],'graphics'))return;await loadJobs();}
async function loadJobs(){const {data,error}=await supabaseClient.from('applications').select('*').is('deleted_at',null).order('created_at',{ascending:false});if(error){document.getElementById('graphicsJobs').textContent=error.message;return;}const rows=(data||[]).filter(r=>r.assigned_to===profile.user_id||terms.some(t=>String(r.department||r.service||'').toLowerCase().includes(t.toLowerCase())));document.getElementById('graphicsAssigned').textContent=rows.length;document.getElementById('graphicsProcessing').textContent=rows.filter(r=>r.status==='Processing').length;document.getElementById('graphicsReview').textContent=rows.filter(r=>['Review','Awaiting Review','Blocked'].includes(r.status)).length;document.getElementById('graphicsCompleted').textContent=rows.filter(r=>r.status==='Completed').length;document.getElementById('graphicsJobs').innerHTML=rows.map(r=>card(r,`<button onclick="setStatus('${r.id}','Processing')">Start</button><button onclick="setStatus('${r.id}','Awaiting Review')">Send Review</button><button onclick="setStatus('${r.id}','Completed')">Complete</button><button onclick="note('${r.id}')">Note</button>`)).join('')||'<p>No assigned jobs.</p>';}
