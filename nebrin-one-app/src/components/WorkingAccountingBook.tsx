...
export function WorkingAccountingBook({book,roleCode,onChanged}:{book:Book;roleCode:string;onChanged?:()=>Promise<void>|void}){
 ...
 const canUseAI=['CASHIER','ACCOUNTANT','TREASURER'].includes(roleCode);
 ...
 async function load(){
  try{setBusy('load');setMsg(null);const r=await getFinanceAccountingBookDetail(book.key);setDetail(r.data||{})}
 ...
 async function createEntry(){
  try{
   setBusy('entry');setMsg(null);
   await createFinanceBookEntry(book.key,date,reference.trim(),description.trim(),Number(debit||0),Number(credit||0),accountCode.trim(),counterparty.trim());
   setReference('');setDescription('');setAccountCode('');setCounterparty('');setDebit('');setCredit('');
   await load();if(onChanged)await onChanged();setMsg('Draft accounting line recorded successfully.');
  }catch(e){setMsg(e instanceof Error?e.message:String(e))}finally{setBusy(null)}
 }
 async function askAI(){
  try{setBusy('ai');setMsg(null);const r=await askFinanceAccountingAI(book.key,aiTask.trim());setAiDraft(r.data||null)}
  catch(e){setMsg(e instanceof Error?e.message:String(e))}finally{setBusy(null)}
 }
 async function acceptAI(){
  if(!aiDraft?.id)return;
  try{
   setBusy('accept-ai');setMsg(null);await acceptFinanceAIDraft(aiDraft.id);setAiDraft(null);setAiTask('');
   await load();if(onChanged)await onChanged();setMsg('AI draft accepted as a Draft accounting line. It still requires normal human review/posting controls.');
  }catch(e){setMsg(e instanceof Error?e.message:String(e))}finally{setBusy(null)}
 }

 return <View style={s.box}>
  <View style={s.header}>
   <View style={{flex:1}}><Text style={s.title}>{book.title}</Text><Text style={s.note}>{isReviewOnly?'Generated / review book':'Live accounting workbook'} · {roleCode.replaceAll('_',' ')}</Text></View>
   <Pressable style={s.refresh} onPress={load}><Text style={s.refreshText}>Refresh</Text></Pressable>
  </View>
  <View style={s.metrics}>
   <Metric label="Debit" value={Number(summary.total_debit||0)}/>
   <Metric label="Credit" value={Number(summary.total_credit||0)}/>
   <Metric label="Balance" value={Number(summary.balance||0)}/>
   <Metric label="Lines" value={Number(summary.working_count||0)}/>
  </View>
  ...
  {canDraft?<Drop title="New Accounting Entry" open={entryOpen} onPress={()=>setEntryOpen(v=>!v)}>
   ...
   <Text style={s.note}>Entries are created as Draft so review and posting controls remain in place.</Text>
  ...
  {canUseAI?<Drop title="AI Accounting Assistant" open={aiOpen} onPress={()=>setAiOpen(v=>!v)}>
   <Text style={s.aiIntro}>Ask for accounting help inside this book: classify a transaction, prepare a journal draft, check debit/credit treatment, explain reconciliation, summarize records, or prepare a draft record. AI suggestions never post automatically.</Text>
   ...
   {aiDraft?<View style={s.aiDraft}>
    <Text style={s.aiTitle}>AI Draft — Human Review Required</Text>
    <KV k="Date" v={ai.entry_date}/><KV k="Reference" v={ai.reference}/><KV k="Description" v={ai.description}/><KV k="Account code" v={ai.account_code}/><KV k="Counterparty" v={ai.counterparty}/><KV k="Debit" v={money(ai.debit)}/><KV k="Credit" v={money(ai.credit)}/>
    <Text style={s.aiLabel}>Accounting explanation</Text><Text style={s.aiText}>{ai.explanation||'—'}</Text>
    {Array.isArray(ai.checks)&&ai.checks.length>0?<View><Text style={s.aiLabel}>Checks before accepting</Text>{ai.checks.map((x:string,i:number)=><Text key={i} style={s.check}>• {x}</Text>)}</View>:null}
    {canDraft?<Pressable disabled={!!busy} style={[s.primary,!!busy&&s.disabled]} onPress={acceptAI}><Text style={s.white}>{busy==='accept-ai'?'Accepting…':'Accept as Draft Entry'}</Text></Pressable>:<Text style={s.note}>This book is review-only; use the AI explanation without creating an entry.</Text>}
   </View>:null}
  </Drop>:null}

  <View style={s.drop}>
   <View style={s.dropHead}><Text style={s.dropTitle}>Working Book Lines ({working.length})</Text></View>
   <View style={s.dropBody}><ScrollView horizontal showsHorizontalScrollIndicator={true}><View style={s.table}><TableHead/>{working.map((x:any,i:number)=><View key={x.id||i} style={s.tr}><Cell w={90} text={fmtDate(x.entry_date)}/><Cell w={105} text={x.reference||'—'}/><Cell w={220} text={x.description||'—'}/><Cell w={90} text={x.account_code||'—'}/><Cell w={120} text={x.counterparty||'—'}/><Cell w={115} text={money(x.debit)}/><Cell w={115} text={money(x.credit)}/><Cell w={90} text={x.status||'Draft'}/></View>)}</View></ScrollView>{!working.length?<Text style={s.note}>No working-book lines yet.</Text>:null}</View>
  </View>

  <Drop title={`Linked Source Records (${Number(summary.source_count||0)})`} open={sourceOpen} onPress={()=>setSourceOpen(v=>!v)}>
   <Text style={s.note}>These are synchronized source records from the NEBRIN Finance system.</Text>
