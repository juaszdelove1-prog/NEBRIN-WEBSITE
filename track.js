...
btn.addEventListener('click',async()=>{
 ...
 resultBox.innerHTML='';
 document.getElementById('paymentRequestBox')?.classList.add('hidden');
 document.getElementById('paymentUploadBox')?.classList.add('hidden');
 ...
 if(reference.startsWith('APT-')){
   show('Checking your appointment booking…','success');
   const {data,error}=await supabaseClient.rpc('track_appointment',{p_reference:reference,p_phone:phone});
   if(error){console.error(error);show('Unable to check the booking at the moment.','error');return;}
   const booking=Array.isArray(data)?data[0]:data;
   window.currentTrackedApplication=null;
   if(!booking){show('No matching appointment booking was found.','error');return;}
   ...
   resultBox.innerHTML=`<article class="track-result-card">
     <div class="track-result-grid">
       ...
       <div><small>Time</small><strong>${esc(booking.appointment_time)}</strong></div>
       <div><small>Submitted</small><strong>${new Date(booking.created_at).toLocaleString()}</strong></div>
     </div>
 ...
 show('Checking your service application…','success');
